...
bool OMXReader::Open(std::string filename, bool dump_format)
{
  ...
  else
  {
    ...
    if (m_dllAvFormat.avformat_open_input(&m_pFormatContext, m_filename.c_str(), iformat, NULL) < 0)
    {
      Close();
      return false;
    }
...
void OMXReader::GetChapterName(std::string& strChapterName)
{
  ...
  AVDictionaryEntry *titleTag = m_dllAvUtil.av_dict_get(m_pFormatContext->chapters[chapterIdx-1]->metadata, "title", NULL, 0);
  if (titleTag)
