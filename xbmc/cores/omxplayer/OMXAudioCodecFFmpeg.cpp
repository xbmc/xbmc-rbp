...
#include "OMXAudioCodecFFmpeg.h"
#ifdef _LINUX
#include "XMemUtils.h"
#endif
#include "utils/log.h"

COMXAudioCodecFFmpeg::COMXAudioCodecFFmpeg()
{
  m_iBufferSize1 = 0;
...
bool COMXAudioCodecFFmpeg::Open(COMXStreamInfo &hints)
{
  AVCodec* pCodec;
  m_bOpenedCodec = false;
  ...
  if (!pCodec)
  {
    CLog::Log(LOGDEBUG,"COMXAudioCodecFFmpeg::Open() Unable to find codec %d", hints.codec);
    return false;
  }
  ...
  {
    CLog::Log(LOGDEBUG,"COMXAudioCodecFFmpeg::Open() Unable to open codec");
    Dispose();
    return false;
  }
...
void COMXAudioCodecFFmpeg::Dispose()
{
  if (m_pConvert)
  {
...
int COMXAudioCodecFFmpeg::Decode(BYTE* pData, int iSize)
{
  int iBytesUsed;
  ...
  if (iBytesUsed > iSize)
  {
    CLog::Log(LOGWARNING, "COMXAudioCodecFFmpeg::Decode - decoder attempted to consume more data than given");
    iBytesUsed = iSize;
  }
  ...
  if(m_pCodecContext->sample_fmt != AV_SAMPLE_FMT_S16 && m_iBufferSize1 > 0)
  {
    ...
    if(!m_pConvert)
    {
      CLog::Log(LOGERROR, "COMXAudioCodecFFmpeg::Decode - Unable to convert %d to AV_SAMPLE_FMT_S16", m_pCodecContext->sample_fmt);
      m_iBufferSize1 = 0;
      m_iBufferSize2 = 0;
      return iBytesUsed;
    }
    ...
    {
      CLog::Log(LOGERROR, "COMXAudioCodecFFmpeg::Decode - Unable to convert %d to AV_SAMPLE_FMT_S16", (int)m_pCodecContext->sample_fmt);
      m_iBufferSize1 = 0;
      m_iBufferSize2 = 0;
      return iBytesUsed;
    }
...
int COMXAudioCodecFFmpeg::GetData(BYTE** dst)
{
  if(m_iBufferSize1)
  {
    *dst = m_pBuffer1;
...
void COMXAudioCodecFFmpeg::Reset()
{
...
int COMXAudioCodecFFmpeg::GetChannels()
{
  return m_pCodecContext->channels;
}

int COMXAudioCodecFFmpeg::GetSampleRate()
{
  if (m_pCodecContext) return m_pCodecContext->sample_rate;
  return 0;
}

int COMXAudioCodecFFmpeg::GetBitsPerSample()
{
  return 16;
}

int COMXAudioCodecFFmpeg::GetBitRate()
{
...
void COMXAudioCodecFFmpeg::BuildChannelMap()
{
  ...
  {
    CLog::Log(LOGINFO, "COMXAudioCodecFFmpeg::GetChannelMap - FFmpeg reported %d channels, but the layout contains %d ignoring", m_pCodecContext->channels, bits);
...
enum PCMChannels* COMXAudioCodecFFmpeg::GetChannelMap()
{
  BuildChannelMap();
