...
#include "OMXAudioRender.h"
#include "utils/log.h"

#define CLASSNAME "COMXAudioRender"
...
//////////////////////////////////////////////////////////////////////
//***********************************************************************************************
COMXAudioRender::COMXAudioRender() :
  m_pCallback       (NULL   ),
  m_Initialized     (false  ),
  m_Pause           (false  ),
  m_CanPause        (false  ),
...
bool COMXAudioRender::Initialize(IAudioCallback* pCallback, const CStdString& device, int iChannels, enum PCMChannels *channelMap, unsigned int uiSamplesPerSec, unsigned int uiBitsPerSample, bool bResample, bool bIsMusic, EEncoded bPassthrough, bool bUseHWDecode, CodecID codec)
{
  m_HWDecode = false;
  m_Passthrough = false;
  ...
  */
  ...
  return Initialize(pCallback, device, iChannels, channelMap, uiSamplesPerSec, uiBitsPerSample, false, false, bPassthrough);
}

bool COMXAudioRender::Initialize(IAudioCallback* pCallback, const CStdString& device, int iChannels, enum PCMChannels *channelMap, unsigned int uiSamplesPerSec, unsigned int uiBitsPerSample, bool bResample, bool bIsMusic, EEncoded bPassthrough)
{
  ...
  if (!m_Passthrough && channelMap && !m_HWDecode)
  {
    ...
    if (m_remap.CanRemap())
    {
      ...
      if (m_DataChannels != (unsigned int)iChannels)
        CLog::Log(LOGDEBUG, "COMXAudioRender:::Initialize Requested channels changed from %i to %i", m_DataChannels, iChannels);
    }
  ...
  if(m_Passthrough)
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "COMXAudioRender::Initialize - Error OMX_IndexParamBrcmDecoderPassThrough 0x%08x", omx_err);
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "COMXAudioRender::Initialize error get OMX_IndexParamPortDefinition omx_err(0x%08x)\n", omx_err);
    return false;
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "COMXAudioRender::Initialize error set OMX_IndexParamPortDefinition omx_err(0x%08x)\n", omx_err);
    return false;
  }
  ...
  if(m_HWDecode)
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "COMXAudioRender::Initialize error OMX_IndexParamAudioPortFormat omx_err(0x%08x)\n", omx_err);
      return false;
    }
  }
  ...
  if(m_av_clock == NULL)
  {
    ...
    if(!m_av_clock->Initialize(false, true))
    {
      delete m_av_clock;
      m_av_clock = NULL;
      CLog::Log(LOGERROR, "COMXAudioRender::Initialize error creating av clock\n");
      return false;
    }
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "COMXAudioRender::Initialize m_omx_tunnel_clock.Establish\n");
    return false;
  }
  ...
  if(!m_external_clock)
  {
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "COMXAudioRender::Initialize m_omx_clock.SetStateForComponent\n");
      return false;
    }
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "COMXAudioRender::Initialize OMX_IndexParamAudioPcm omx_err(0x%08x)\n", omx_err);
    return false;
  }
  */
  ...
  if(omx_err != OMX_ErrorNone) 
  {
    CLog::Log(LOGERROR, "COMXAudioRender::Initialize - Error alloc buffers 0x%08x", omx_err);
    return false;
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "COMXAudioRender::Initialize - Error m_omx_tunnel_decoder.Establish 0x%08x", omx_err);
    return false;
  }

  omx_err = m_omx_decoder.SetStateForComponent(OMX_StateExecuting);
  if(omx_err != OMX_ErrorNone) {
    CLog::Log(LOGERROR, "COMXAudioRender::Initialize - Error setting OMX_StateExecuting 0x%08x", omx_err);
    return false;
  }
  ...
  if(m_eEncoding == OMX_AUDIO_CodingPCM)
  {
    ...
    if(omx_buffer == NULL)
    {
      CLog::Log(LOGERROR, "COMXAudioRender::Initialize - buffer error 0x%08x", omx_err);
      return false;
    }
    ...
    {
      CLog::Log(LOGERROR, "COMXAudioRender::Initialize - omx_buffer->nFilledLen > omx_buffer->nAllocLen");
      return false;
    }
  ...
  CLog::Log(LOGDEBUG, "COMXAudioRender::Initialize bps %d samplerate %d channels %d device %s buffer size %d bytes per second %d passthrough %d hwdecode %d", 
...
//***********************************************************************************************
bool COMXAudioRender::Deinitialize()
{
  if(!m_Initialized)
    return true;
...
//***********************************************************************************************
long COMXAudioRender::GetCurrentVolume() const
{
  return m_CurrentVolume;
}

//***********************************************************************************************
void COMXAudioRender::Mute(bool bMute)
{
  if(!m_Initialized)
    return;
...
//***********************************************************************************************
bool COMXAudioRender::SetCurrentVolume(long nVolume)
{
...
//***********************************************************************************************
unsigned int COMXAudioRender::GetSpace()
{
  int free = m_omx_decoder.GetInputBufferSpace();
  return (free / m_Channels) * m_DataChannels;
}

unsigned int COMXAudioRender::AddPackets(const void* data, unsigned int len)
{
  return AddPackets(data, len, 0);
}

//***********************************************************************************************
unsigned int COMXAudioRender::AddPackets(const void* data, unsigned int len, double pts)
{
  if(!m_Initialized) {
    CLog::Log(LOGERROR,"COMXAudioRender::AddPackets - sanity failed. no valid play handle!");
    return len;
  }
  ...
  while(demuxer_bytes)
  {
    ...
    if(omx_buffer == NULL)
    {
      CLog::Log(LOGERROR, "COMXAudioRender::Decode timeout\n");
      printf("COMXAudioRender::Decode timeout\n");
      return len;
    }
    ...
    */
    ...
    if(m_first_frame)
    {
      ...
      if(!m_Passthrough)
      {
        ...
        if(omx_err != OMX_ErrorNone)
        {
          CLog::Log(LOGERROR, "COMXAudioRender::AddPackets error GetParameter 1 omx_err(0x%08x)\n", omx_err);
        }
        ...
        if(omx_err != OMX_ErrorNone)
        {
          CLog::Log(LOGERROR, "COMXAudioRender::AddPackets error SetParameter 1 omx_err(0x%08x)\n", omx_err);
        }
        omx_err = m_omx_render.GetParameter(OMX_IndexParamAudioPcm, &m_pcm);
        if(omx_err != OMX_ErrorNone)
        {
          CLog::Log(LOGERROR, "COMXAudioRender::AddPackets error GetParameter 2  omx_err(0x%08x)\n", omx_err);
        }
      ...
      else
      {
        ...
        if(omx_err != OMX_ErrorNone)
        {
          CLog::Log(LOGERROR, "COMXAudioRender::AddPackets error OMX_IndexParamAudioPortFormat omx_err(0x%08x)\n", omx_err);
          assert(0);
        }
...
//***********************************************************************************************
float COMXAudioRender::GetDelay()
{
...
float COMXAudioRender::GetCacheTime()
{
  /*
...
float COMXAudioRender::GetCacheTotal()
{
  return (float)m_BufferLen / (float)m_BytesPerSec;
}

//***********************************************************************************************
unsigned int COMXAudioRender::GetChunkLen()
{
  return (m_ChunkLen / m_Channels) * m_DataChannels;
}
//***********************************************************************************************
int COMXAudioRender::SetPlaySpeed(int iSpeed)
{
  return 0;
}

void COMXAudioRender::RegisterAudioCallback(IAudioCallback *pCallback)
{
...
void COMXAudioRender::UnRegisterAudioCallback()
{
  m_pCallback = NULL;
}

void COMXAudioRender::DoAudioWork()
{
...
void COMXAudioRender::WaitCompletion()
{
...
void COMXAudioRender::SwitchChannels(int iAudioStream, bool bAudioOnAllSpeakers)
{
    return ;
}

void COMXAudioRender::EnumerateAudioSinks(AudioSinkList& vAudioSinks, bool passthrough)
{
#ifndef STANDALONE
  if (!passthrough)
  {
...
bool COMXAudioRender::SetClock(OMXClock *clock)
{
  if(m_av_clock != NULL)
    return false;
...
void COMXAudioRender::SetEncodedType(EEncoded encoded)
{
  switch(encoded)
  { 
    case IAudioRenderer::ENCODED_IEC61937_UNKNOWN:
    case IAudioRenderer::ENCODED_IEC61937_DTS:
      CLog::Log(LOGDEBUG, "COMXAudioRender::SetEncodedType OMX_AUDIO_CodingDTS\n");
      m_eEncoding = OMX_AUDIO_CodingDTS;
      break;
    case IAudioRenderer::ENCODED_IEC61937_AC3:
    case IAudioRenderer::ENCODED_IEC61937_EAC3:
      CLog::Log(LOGDEBUG, "COMXAudioRender::SetEncodedType OMX_AUDIO_CodingDDP\n");
      m_eEncoding = OMX_AUDIO_CodingDDP;
      break;
    default:
      CLog::Log(LOGDEBUG, "COMXAudioRender::SetEncodedType OMX_AUDIO_CodingPCM\n");
      m_eEncoding = OMX_AUDIO_CodingPCM;
      break;
  } 
}

void COMXAudioRender::SetCodingType(CodecID codec)
{
  switch(codec)
  { 
    case CODEC_ID_DTS:
...
bool COMXAudioRender::CanHWDecode(CodecID codec)
{
  switch(codec)
  { 
    case CODEC_ID_MP1:
    case CODEC_ID_MP2:
    case CODEC_ID_MP3:
      CLog::Log(LOGDEBUG, "COMXAudioRender::CanHWDecode OMX_AUDIO_CodingMP3\n");
      m_eEncoding = OMX_AUDIO_CodingMP3;
      m_HWDecode = true;
      break;
    case CODEC_ID_DTS:
      CLog::Log(LOGDEBUG, "COMXAudioRender::CanHWDecode OMX_AUDIO_CodingDTS\n");
      m_eEncoding = OMX_AUDIO_CodingDTS;
      m_HWDecode = true;
      break;
    case CODEC_ID_AC3:
    case CODEC_ID_EAC3:
      CLog::Log(LOGDEBUG, "COMXAudioRender::CanHWDecode OMX_AUDIO_CodingDDP\n");
      m_eEncoding = OMX_AUDIO_CodingDDP;
      m_HWDecode = true;
      break;
    default:
      CLog::Log(LOGDEBUG, "COMXAudioRender::CanHWDecode OMX_AUDIO_CodingPCM\n");
      m_eEncoding = OMX_AUDIO_CodingPCM;
      m_HWDecode = false;
      break;
  } 
...
bool COMXAudioRender::HWDecode(CodecID codec)
{
  bool ret = false;
  ...
  { 
    ...
    case CODEC_ID_VORBIS:
      CLog::Log(LOGDEBUG, "COMXAudioRender::HWDecode CODEC_ID_VORBIS\n");
      ret = true;
      break;
    case CODEC_ID_AAC:
      CLog::Log(LOGDEBUG, "COMXAudioRender::HWDecode CODEC_ID_AAC\n");
      ret = true;
      break;
    */
    case CODEC_ID_MP2:
    case CODEC_ID_MP3:
      CLog::Log(LOGDEBUG, "COMXAudioRender::HWDecode CODEC_ID_MP2 / CODEC_ID_MP3\n");
      ret = true;
      break;
    case CODEC_ID_DTS:
      CLog::Log(LOGDEBUG, "COMXAudioRender::HWDecode CODEC_ID_DTS\n");
      ret = true;
      break;
    case CODEC_ID_AC3:
    case CODEC_ID_EAC3:
      CLog::Log(LOGDEBUG, "COMXAudioRender::HWDecode CODEC_ID_AC3 / CODEC_ID_EAC3\n");
      ret = true;
      break;
    default:
      ret = false;
...
void COMXAudioRender::PrintChannels(OMX_AUDIO_CHANNELTYPE eChannelMapping[])
{
...
void COMXAudioRender::PrintPCM(OMX_AUDIO_PARAM_PCMMODETYPE *pcm)
{
...
void COMXAudioRender::PrintDDP(OMX_AUDIO_PARAM_DDPTYPE *ddparm)
{
...
void COMXAudioRender::PrintDTS(OMX_AUDIO_PARAM_DTSTYPE *dtsparam)
{
...
/* ========================== SYNC FUNCTIONS ========================== */
unsigned int COMXAudioRender::SyncDTS(BYTE* pData, unsigned int iSize)
{
...
unsigned int COMXAudioRender::SyncAC3(BYTE* pData, unsigned int iSize)
{
  unsigned int skip = 0;
